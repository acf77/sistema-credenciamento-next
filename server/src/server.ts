...
// // @PUT update the event status (isEventStarted: true || false)
// app.put("/api/events/start/:id", (req, res) => {
//   const { params } = req;

//   const startEventOnDb = async () => {
//     try {
//       const event = await Event.findById(params.id);
//       const eventStarted = await Event.findByIdAndUpdate(
//         { _id: params.id },
//         { $set: { isEventStarted: !event.isEventStarted } }
//       );
//       res.status(200).json(eventStarted);
//     } catch (error) {
//       res.status(400).json(error);
//     }
//   };
...
// // @PUT add new invitee
// app.put("/api/events/invitee", (req, res) => {
//   const { body } = req;
...
// app.post("/api/qrcode/:event_id/:invitee_id", (req, res) => {
//   const { params } = req;
...
//       // Configures email info and content in html
//       const mailOptions = {
